...
import { X, Lock, TrendingUp, Users, Upload, FileText, AlertCircle, Send, CheckCircle } from 'lucide-react';
      ...
      <div className="bg-[#F9F4EE] w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-3xl shadow-2xl" dir="rtl">
        ...
        <div className="p-6 space-y-6">
          <div className="rounded-2xl p-5 border-2 border-[#C9A84C]" style={{ background: 'rgba(201,168,76,0.06)' }}>
            ...
            <p className="text-4xl font-black mb-2 text-[#0D1B2A]">{formatCurrency(task.payment)}</p>
            <p className="text-sm text-gray-500">✅ الأموال محفوظة في الضمان · ✅ تُصرف مباشرة لك</p>
          </div>

          {!task.paid && (
            <div className="bg-white rounded-2xl p-5 border border-[#E8DDD0]">
              <h3 className="font-bold text-sm text-gray-500 uppercase mb-4 flex items-center gap-2"><Upload size={14} /> تسليم العمل وطلب الدفع</h3>
              {requestSent ? (
                ...
                <div className="space-y-4">
                  <label className="flex flex-col items-center justify-center w-full h-28 border-2 border-dashed border-[#E8DDD0] rounded-xl cursor-pointer bg-[#F9F4EE] hover:border-[#C9A84C] transition-colors">
                    ...
                    <p className="text-sm text-gray-400">{uploadedFile || 'اضغط لرفع ملف التسليم'}</p>
                  </label>
                  <button 
                    disabled={!uploadedFile}
                    onClick={handleSubmit}
                    className="w-full py-4 rounded-xl font-black text-white bg-[#C9A84C] disabled:bg-gray-200 disabled:text-gray-400 flex items-center justify-center gap-2 shadow-lg"
                  >
                    <Send size={18} /> إرسال العمل وطلب مستحقاتي
                  </button>
                </div>
              )}
            </div>
