...
export const TrustScore: FC<TrustScoreProps> = ({
  score,
  label = 'Trust Score',
  className,
  size = 'md',
  ...
  return (
    <div
      className={cn(
        'flex flex-col gap-2 rounded-3xl border-2 p-4 transition-all hover:shadow-md',
        bgColor,
        borderColor,
        className
      )}
    >
      <div className="flex items-center justify-between">
        ...
        {isHigh ? (
          <ShieldCheck size={14} className={statusColor} />
        ) : (
      ...
      <div className="flex items-end gap-2">
        ...
        <div className="mb-1 flex flex-col gap-0.5">
          <div className="flex items-center gap-1 text-[9px] font-black tracking-tighter text-emerald-600 uppercase">
      ...
      <div className="h-1 w-full overflow-hidden rounded-full bg-gray-200/50">
        <motion.div
          ...
          className={cn(
            ...
            isHigh ? 'bg-emerald-500' : isMed ? 'bg-[#C9A84C]' : 'bg-red-500'
          )}
        />
      </div>
    </div>
