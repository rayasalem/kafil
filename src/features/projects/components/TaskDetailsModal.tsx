...
export const TaskDetailsModal: FC<TaskDetailsModalProps> = ({ task, project, onClose, onApprove, userRole }) => {
  const queryClient = useQueryClient();
  const submitWorkMutation = useMutation({
    mutationFn: (fileName: string) => {
       // Using project.id if passed, else search for it among all projects
       return api.getProjects().then(projects => {
          let pId = project?.id;
          if (!pId) {
            const p = projects.find(p => p.tasks.some(t => t.id === task.id));
            if(p) pId = p.id;
          }
          if (!pId) throw new Error('Project not found for task');
          return api.submitTaskWork(pId, task.id, { deliverableFile: fileName });
       });
    },
    onSuccess: () => {
       toast.success('تم تسليم العمل بنجاح');
        ...
        <div className="p-6 space-y-6">
          ...
          <div>
            ...
            {task.deliverableFile ? (
              ...
              userRole === 'freelancer' ? (
                <div className="p-8 text-center border-2 border-dashed border-[#C9A84C] bg-[#C9A84C]/5 rounded-xl text-[#0D1B2A] cursor-pointer hover:bg-[#C9A84C]/10 transition-colors"
                  onClick={() => {
                      const fileName = prompt('الرجاء إدخال اسم الملف (للمحاكاة فقط):', 'deliverable_v1.zip');
                      if (fileName) {
                        submitWorkMutation.mutate(fileName);
                      }
                    }}
                  >
                    <Upload size={24} className="mx-auto mb-2 text-[#C9A84C]" />
                    <p className="font-bold text-sm">اضغط هنا لرفع الملفات (محاكاة)</p>
                    {submitWorkMutation.isPending && <p className="text-xs text-gray-500 mt-2">جاري الرفع...</p>}
                </div>
              ) : (
