      ...
      return new Promise((resolve) => {
        setTimeout(() => {
          resolve({
            freelancerShare: 75,
            clientShare: 25,
            explanation: "Based on the delivery notes and the provided file architecture, 75% of requirements were met. The remaining 25% requires minor visual adjustments which don't block core functionality. Suggesting 75/25 split.",
            confidence: 94
          });
        }, 2000);
      });
      ...
      const response = await ai.models.generateContent({
        ...
        config: {
          ...
          responseSchema: {
            type: Type.OBJECT,
            properties: {
              freelancerShare: { type: Type.NUMBER, description: "Percentage for freelancer (0-100)" },
              clientShare: { type: Type.NUMBER, description: "Percentage for client (0-100)" },
              explanation: { type: Type.STRING, description: "Professional explanation for the split" },
              confidence: { type: Type.NUMBER, description: "Confidence score in analysis (0-100)" }
            },
