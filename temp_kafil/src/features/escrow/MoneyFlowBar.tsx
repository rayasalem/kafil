  ...
  return (
    <div className="w-full">
      <div className="flex justify-between text-xs font-black text-gray-400 uppercase tracking-widest mb-3">
        ...
        <span>الرصيد المتاح: <span className="text-blue-600">{formatCurrency(remaining)}</span></span>
      </div>
      <div className="h-4 w-full bg-gray-100 rounded-full overflow-hidden flex shadow-inner">
        <div 
          className="h-full bg-emerald-500 transition-all duration-1000 ease-out" 
          style={{ width: `${paidWidth}%` }}
          title={`تم صرفه: ${formatCurrency(paid)}`}
        />
        <div 
          className="h-full bg-blue-500 transition-all duration-1000 ease-out opacity-80" 
          style={{ width: `${allocatedWidth}%` }}
          title={`محجوز بالخزنة: ${formatCurrency(allocated - paid)}`}
        />
      </div>
      <div className="flex flex-wrap gap-4 md:gap-6 mt-4">
        <div className="flex items-center gap-2">
          <span className="w-3 h-3 rounded-sm bg-emerald-500"></span>
          <span className="text-sm font-bold text-gray-600">تم صرفه ({formatCurrency(paid)})</span>
        </div>
        <div className="flex items-center gap-2">
          <span className="w-3 h-3 rounded-sm bg-blue-500"></span>
          <span className="text-sm font-bold text-gray-600">محجوز بالخزنة ({formatCurrency(allocated - paid)})</span>
        </div>
        <div className="flex items-center gap-2">
          <span className="w-3 h-3 rounded-sm bg-gray-200"></span>
          <span className="text-sm font-bold text-gray-600">متاح للمهام القادمة ({formatCurrency(remaining)})</span>
        </div>
      </div>
    </div>
  );
